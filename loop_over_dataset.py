...
exec_detection = []
#exec_detection = ['bev_from_pcl', 'pcl_from_rangeimage', 'detect_objects','validate_object_labels', 'measure_detection_performance'] # options are 'bev_from_pcl', 'detect_objects', 'validate_object_labels', 'measure_detection_performance'; options not in the list will be loaded from file
...
while True:
    try:
        ...
        if 'measure_detection_performance' in exec_list:
            ...
            det_performance = eval.measure_detection_performance(detections, frame.laser_labels, valid_label_flags, 0.5 ) #configs_det.min_iou)     
        else:
        ...
        if 'show_range_image' in exec_list:
            ...
            cv2.imshow('stacked_image', img_range)
            cv2.waitKey(vis_pause_time)
