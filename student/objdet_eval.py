...
def compute_performance_stats(det_performance_all, configs_dets):
    ...
    s_pos_negs = np.sum(pos_negs, axis=0)
    p = np.sum(s_pos_negs[0])
    tp = np.sum(s_pos_negs[1])
    fn = np.sum(s_pos_negs[2])
    fp = np.sum(s_pos_negs[3])
